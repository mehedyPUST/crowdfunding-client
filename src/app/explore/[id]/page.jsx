...
export default function SupporterExplorePage() {
    ...
    useEffect(() => {
        api.get('/campaigns').then((res) => { setCampaigns(res.data); setFiltered(res.data); }).catch(() => { }).finally(() => setLoading(false));
    }, []);
    ...
    if (loading) {
        return (
            <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row gap-3"><Skeleton height={42} className="flex-1" borderRadius={12} /><Skeleton width={160} height={42} borderRadius={12} /></div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">{[...Array(6)].map((_, i) => <CampaignCardSkeleton key={i} />)}</div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div><h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Explore Campaigns</h1><p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Discover and support projects that matter</p></div>
                <div className="flex items-center gap-2 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl px-5 py-3 shadow-sm">
                    <Coins className="w-5 h-5 text-amber-500" /><div><p className="text-xs text-gray-500 dark:text-gray-400">Your Credits</p><p className="text-lg font-bold text-amber-700 dark:text-amber-400">🪙 {user?.credits || 0}</p></div>
                </div>
            </motion.div>
